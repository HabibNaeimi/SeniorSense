...
class Registration_deployer(object):
    ...
    def GET(self,*uri,**params):
        ...
        #get patients list associated with thingspeak channel
        elif (len(uri))>0 and uri[0]=="associated_patients":
    ...
    def PUT(self,*uri):
        ...
        if command=="newPlatform":
            ...
            output=self.catalog.platforms.associate_patient_thingspeak(json_body['platformID'],json_body['patientID'])
            if output is not False:
                self.catalog.platforms.save()
                if output=="Available thingspeak channel found! Registered...":
                    grafana_catalog=self.catalog.retrieveService('grafana_catalog')
                    log=requests.post(grafana_catalog['url']+"/"+json_body['platformID']+"/"+json_body['patientID']+"/createDashboard")
                print("{} - {}".format(json_body['platformID'],json_body['patientID']))
                print(output)
    ...
    def DELETE(self,*uri):
        ...
        if command=='removePlatform':
            ...
            if self.catalog.users.find_user(username) and platform_ID in self.catalog.users.find_user(username)["platforms_list"]:
                outputFlag=self.catalog.platforms.removepatient(platform_ID,patient_ID)
                if outputFlag:
                    ...
                    log=requests.delete(grafana_catalog['url']+"/"+platform_ID+"/"+patient_ID+"/deleteDashboard/"+org_key)
